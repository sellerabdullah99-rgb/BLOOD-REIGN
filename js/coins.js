...
BR.coins = (function () {
  ...
  // This 15-second wait before "Claim now" is an honesty gate — most ad
  // networks can't confirm an ad was actually watched, so this at least
  // stops an instant no-look claim once a real ad network is wired in.
  //
  // ⚠️ TODO: real rewarded-ad network setup is PENDING (switching to
  // Monetag — Adsterra was tried and removed after it served a scammy
  // "FB hacked" push notification). Once you have Monetag's ad script,
  // put its URL in config.js as AD_NETWORK_SCRIPT_SRC and load it at page
  // load in index.html — avoid the "Push Notifications" format.
  function handleWatchAd(anchorEl) {
    const src = BR.config.AD_NETWORK_SCRIPT_SRC;
    if (!src || src.includes('YOUR-')) {
      toast('Ad network not connected yet — see the TODO in config.js', 'default', 'fa-triangle-exclamation');
      return;
    }
