...
BR.profile = (function () {
  ...
  function openEditModal(profile) {
    BR.ui.openModal('editProfileModal', {
      ...
      bodyHTML: `
        ...
        <input class="input" id="editIgn" value="${escapeHtml(profile.ign || '')}" placeholder="e.g. FireStorm_PK" style="margin-bottom:12px">
        <label class="field-label">Free Fire UID</label>
        <input class="input" id="editFfUid" value="${escapeHtml(profile.ff_uid || '')}" placeholder="e.g. 123456789" inputmode="numeric" style="margin-bottom:12px">
        <label class="field-label">Your Role</label>
        <select class="select" id="editRole" style="margin-bottom:16px">
          ${['IGL','RUSHER','ASSAULTER','SNIPER','SUPPORT','SUBSTITUTE'].map(r => `<option value="${r}" ${profile.primary_role === r ? 'selected' : ''}>${r.charAt(0) + r.slice(1).toLowerCase()}</option>`).join('')}
        </select>
  ...
  function renderAuthForm(kind) {
    ...
    wrap.innerHTML = `
      ${kind === 'signup' ? `
        <label class="field-label">Username</label>
        <input class="input" id="authUsername" placeholder="FireStorm_PK" style="margin-bottom:10px">
        <label class="field-label">In-game Username (IGN)</label>
        <input class="input" id="authIgn" placeholder="e.g. FireStorm_PK" style="margin-bottom:10px">
        <label class="field-label">Free Fire UID</label>
        <input class="input" id="authFfUid" placeholder="e.g. 123456789" inputmode="numeric" style="margin-bottom:10px">
        <label class="field-label">Your Role</label>
        <select class="select" id="authRole" style="margin-bottom:10px">
          <option value="IGL">IGL (In-Game Leader)</option>
          <option value="RUSHER">Rusher</option>
          <option value="ASSAULTER" selected>Assaulter</option>
      ...
      ${kind === 'signup' ? `<p class="muted" style="font-size:var(--text-xs);margin-top:10px"><i class="fa-solid fa-circle-info"></i> This becomes your identity across tournaments, teams &amp; tryouts — you can still tweak it later in Edit Profile.</p>` : ''}
    `;
    $('#authSubmitBtn').addEventListener('click', async () => {
      ...
      if (kind === 'signup') {
        ...
        const ign = $('#authIgn').value.trim() || username;
        const ffUid = $('#authFfUid').value.trim();
        const role = $('#authRole').value;
        if (!ffUid) { toast('Enter your Free Fire UID', 'default', 'fa-triangle-exclamation'); return; }
        res = await BR.auth.signUp(email, password, username, { ign, ffUid, role });
        if (res.ok) await BR.auth.updateProfileFields({ ign, ff_uid: ffUid, primary_role: role });
      } else {
        res = await BR.auth.signIn(email, password);
      }
