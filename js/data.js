...
BR.data = (function () {
  ...
  async function getTournaments() {
    ...
    const merged = [...BR.mockData.tournaments, ...local.addedTournaments]
      ...
      .map(t => local.winnerOverrides[t.id]
        ...
        : (local.statusOverrides && local.statusOverrides[t.id]) ? { ...t, status: local.statusOverrides[t.id] } : t);
    return merged;
  }
  ...
  // Admin view — includes id (needed to delete) and inactive ones too.
  async function adminGetAllAnnouncements() {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.from('announcements').select('id, message, active, created_at').order('created_at', { ascending: false });
      if (error) { console.error(error); return []; }
      return data;
    }
    const local = _adminLocal();
    return local.addedAnnouncements.map((message, i) => ({ id: `local_${i}`, message, active: true }));
  }

  async function deleteAnnouncement(id) {
    if (BR.isConfigured) {
      const { error } = await BR.sb.from('announcements').delete().eq('id', id);
      return error ? { ok: false, error: error.message } : { ok: true };
    }
    const local = _adminLocal();
    const idx = parseInt(id.replace('local_', ''));
    if (!isNaN(idx)) local.addedAnnouncements.splice(idx, 1);
    _saveAdminLocal(local);
    return { ok: true };
  }
  ...
  // Moves a tournament through UPCOMING -> LIVE -> COMPLETED. Admin RLS on
  // the tournaments table already permits this direct update.
  async function adminSetTournamentStatus(id, status) {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.from('tournaments').update({ status }).eq('id', id).select();
      if (error) return { ok: false, error: error.message };
      if (!data || data.length === 0) return { ok: false, error: 'Nothing changed — check is_admin is set in Supabase.' };
      return { ok: true };
    }
    const local = _adminLocal();
    local.statusOverrides = local.statusOverrides || {};
    local.statusOverrides[id] = status;
    _saveAdminLocal(local);
    return { ok: true };
  }
  ...
  // ---------------------------------------------------------
  // ADMIN — TEAMS / TRYOUTS / SCRIMS
  // ---------------------------------------------------------

  // All tryouts regardless of status (OPEN + CLOSED), for admin moderation.
  async function adminGetAllTryouts() {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.from('tryouts').select('*, teams(name, tag), target:profiles!tryouts_target_profile_id_fkey(username)').order('created_at', { ascending: false });
      if (error) { console.error(error); return []; }
      return data.map(t => ({ ...t, team_name: t.teams?.name, team_tag: t.teams?.tag, target_username: t.target?.username }));
    }
    return BR.teamStore.getTryouts();
  }

  async function adminGetTryoutApplications(tryoutId) {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.from('tryout_applications').select('*').eq('tryout_id', tryoutId).order('created_at', { ascending: false });
      if (error) { console.error(error); return []; }
      return data;
    }
    return BR.teamStore.getApplications(tryoutId);
  }

  // Search registered profiles by username, for the admin's "add player
  // to team" tool (profiles are publicly readable, so this needs no RPC).
  async function adminSearchProfiles(query) {
    ...
    return guest.username && guest.username.toLowerCase().includes(query.toLowerCase())
      ? [{ id: 'guest_local', username: guest.username, ff_uid: guest.ff_uid }] : [];
  }

  async function adminGetAllTeams() {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.from('teams').select('id, name, tag').order('name');
      if (error) { console.error(error); return []; }
      return data;
    }
  ...
  // Schedules a PLANNED tryout — a trial with one specific registered
  // player at a set time, distinct from an open "anyone can apply" posting.
  async function adminScheduleTryout(teamId, profileId, role, scheduledTime, notes) {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.rpc('admin_schedule_tryout', {
        p_team_id: teamId, p_profile_id: profileId, p_role: role,
        p_scheduled_time: scheduledTime, p_notes: notes || null,
      });
      if (error) return { ok: false, error: error.message };
      return data;
    }
    return { ok: false, error: 'Connect Supabase to use this in a real multi-user setup.' };
  }

  // Drops a registered/logged-in player straight into a team's roster,
  // bypassing the tryout-application flow (admin only).
  async function adminAddRegisteredPlayerToTeam(teamId, profileId, role) {
    if (BR.isConfigured) {
      const { data, error } = await BR.sb.rpc('admin_add_registered_player_to_team', {
        p_team_id: teamId, p_profile_id: profileId, p_role: role,
      });
      if (error) return { ok: false, error: error.message };
      return data;
  ...
  return {
    ...
    getProducts, getAnnouncements, publishAnnouncement, adminGetAllAnnouncements, deleteAnnouncement, getLeaderboard,
