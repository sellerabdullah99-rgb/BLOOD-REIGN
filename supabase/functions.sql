...
-- ------------------------------------------------------------
-- buy_product_with_coins(): the shop's whole currency — deducts the
-- product's coin_price from the buyer's balance (atomically, server-side
-- so it can't be forged from the client) and creates the order.
-- ------------------------------------------------------------
create or replace function public.buy_product_with_coins(p_product_id uuid, p_delivery_address text)
returns json
language plpgsql
security definer set search_path = public
as $$
...
begin
  ...
  if v_balance < v_product.coin_price then
    return json_build_object('ok', false, 'error', 'insufficient_coins', 'balance', v_balance, 'need', v_product.coin_price);
  end if;

  v_new_bal := public._earn_coins(auth.uid(), -v_product.coin_price, 'Bought: ' || v_product.name);

  insert into public.orders (user_id, product_id, product_name, price, coins_used, discount_pct, delivery_address, status)
    values (auth.uid(), v_product.id, v_product.name, 0, v_product.coin_price, 0, p_delivery_address, 'pending');
